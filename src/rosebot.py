"""
  ...
  Author:  Your professors (for the framework and lower-level code)
    and Aaryan Khatri Joe Law Lucas D'Alesio.
...
class DriveSystem(object):
    ...
    def go_until_distance_is_within(self, delta, inches, speed):
        ...
        self.go(speed, speed)
        while True:
            if abs(inches-delta) & abs(delta+inches) <= self.ir_proximity_sensor:
                self.stop()
                break
...
class ArmAndClaw(object):
    ...
    def calibrate_arm(self):
        ...
        self.motor.turn_on(-100)
        while True:
            if abs(self.motor.get_position()) >= 360*14.2:
                self.motor.turn_off()
                break
        self.motor.reset_position()
    def move_arm_to_position(self, desired_arm_position):
        ...
        if starting_position < int(desired_arm_position):
            self.motor.turn_on(100)
            while True:
                if abs(self.motor.get_position()) >= int(desired_arm_position):
                    self.motor.turn_off()
                    break
        elif starting_position > int(desired_arm_position):
            self.motor.turn_on(-100)
            while True:
                if abs(self.motor.get_position()) <= int(desired_arm_position):
                    self.motor.turn_off()
